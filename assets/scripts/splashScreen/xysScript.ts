...
@ccclass('xysScript')
export class xysScript extends Component {
    @property({
        type: Prefab
    })
    ...
    createGrid(data) {
        ...
        //            let displayedRow = i === 0 ? dl.row : dl.row - 1;
    ...
    getImagePath(row, col, img) {
        return `ndl/points_${img}/spriteFrame`;  // Adjust the path based on your image location
    }
